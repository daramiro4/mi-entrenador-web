...
- **Decisión 4 (capa diaria adaptativa)**: hecho. Se dispara sola al montar el dashboard vía un efecto de cliente que llama a un Server Action (`components/dashboard/DailyAdaptationEffect.tsx` → `app/dashboard/daily-adaptation-actions.ts: ensureDailyAdaptationAction`) — nunca escribe durante el render del Server Component. Lee `fatigue_index` del día exacto de hoy (`lib/metrics.ts: getFatigueIndexForDate`, no "el más reciente"), y aplica `lib/daily-adaptation.ts: computeDailyAdaptation` a la sesión de hoy: normal sin cambios, precaución degrada (quality→z2, o -25% TSS si ya era z2 — nunca cancela), descanso pospone automáticamente al hueco libre más próximo (reutiliza `getValidPostponeDates`, a diferencia de la decisión 3 que es elección manual). Naturalmente idempotente: una vez que la sesión de hoy deja de estar `planned`, no se vuelve a tocar.
...
Aún NO existe:
- El evento de "cierre semanal" que genera la semana siguiente (decisión 1) — hoy solo se genera la semana 1 en el onboarding. Del que dependen la narrativa IA (decisión 7) y parte del sentido de la señal de progreso (decisión 8, que hoy solo cubre la semana ya generada).
- La narrativa semanal vía IA (decisión 7) — bloqueada además por no tener `GEMINI_API_KEY` disponible para `mi-entrenador-web`.
